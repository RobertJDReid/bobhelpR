...
#' @param label Boolean. When \code{label} = \code{TRUE}, the plot displays the angle in the point that
#' acts as the vertex. If missing, it works as with \code{label} = \code{FALSE}, so the angle is not displayed
